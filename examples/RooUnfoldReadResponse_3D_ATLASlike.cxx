...
void RooUnfoldReadResponse_3D_ATLASlike(std::string cFiles2="",std::string tag = "", std::string date = "", int fileindex=0, int year = 2018, std::string mc_name = "", std::string jec_option = "", std::string jer_option ="", std::string pu_option = "", std::string algo = "" )
{
...
// matching efficiency
   ...
   TH3D *h2efficiency(0);
   h2efficiency=new TH3D("efficiency","efficiency",bin_true_lntheta-1, xbins_true, bin_true_lnkt-1,ybins, bin_true_pTjet-1, pT_true);


   //true correlations with det-level cuts
   TH3D *h2true(0);

    h2true=new TH3D("true","true",bin_true_lntheta-1, xbins_true,bin_true_lnkt-1,ybins, bin_true_pTjet-1, pT_true);
    TH3D *h2fulleff(0);

     h2fulleff=new TH3D("truef","truef",bin_true_lntheta-1, xbins_true,bin_true_lnkt-1,ybins, bin_true_pTjet-1, pT_true);
...
//      cout << newCorrection << endl;
    ...
    if ( !(jet1_pT_corrected > 675 ) ) continue;
    if ( jet1.looseID() == false ) continue;
    if ( jet1_pT_corrected/eventHdr.pthat() > 2.5 ) continue;
//    if ( jet1_pT_corrected/eventHdr.pthat() > 1.5 ) continue;
    if ( eventHdr.pthat() < 250.0 ) continue;
//      weight = 1.;
...
//      cout << jet2_pT_corrected << endl;
     ...
     if ( jet1_pT_corrected > 675.0 && jet2_pT_corrected > 450 && fabs(jet1.eta()) < 2.1 && fabs(jet2.eta()) < 2.1 && jet1_pT_corrected < 4000 && jet2_pT_corrected < 4000 && jet1.looseID() == true && jet2.looseID() == true && jet2_pT_corrected/jet1_pT_corrected > 2./3. )
      {
      dijet_pass = true;
      double delta_phi = 0.;
...
//      if  (!(z_gen.size()> 0 && jet_gen.pt() > 500 && fabs(jet_gen.y()) < 2.0 && z_reco.size() > 0 && jetPtCorrected > 550 && fabs(jet_reco.y()) < 2 && jet_reco.looseID() == true && jet_gen.pt() < 4000. && jetPtCorrected < 4000 && fabs(jet_reco.eta()) < 2.0  ) ) continue;
      if( !( dijet_pass == true && jet_gen.pt() > 300 && jetPtCorrected > 466.6 && fabs(jet_reco.eta()) < 2.1 && jet_reco.looseID() == true && jet_gen.pt() < 4000. && jetPtCorrected < 4000  ) ) continue;
...
//      cout << jetPtCorrected << endl;
      ...
      n_jets++;
      n_splittings = 0;
     pT_response.Fill(jetPtCorrected , jet_gen.pt(),  weight);
     pTjet_det->Fill(jetPtCorrected, weight);
     pTjet_true->Fill(jet_gen.pt(),weight);
...
//      cout << "gen-level splitting " << i << endl;
 //     cout << "=================== " << i << endl;
      ...
      if (log(1./z_gen.at(i) ) < 0.693147 || log(1./z_gen.at(i)) > 0.693147+0.27726*19) continue; //gen-level phase-space
      if (log(R/theta_gen.at(i)) < -1. || log(R/theta_gen.at(i)) > logRmaxTrue ) continue; //gen-level phase-space
      if (splitDoesNotPass )  h2fulleff->Fill(log(R/theta_gen.at(i)) , log(1./z_gen.at(i)), jet_gen.pt(), weight );
      n_splittings++;
...
//       cout  << z_reco.at(j) << endl;
      ...
      // cout << " first splitting " << first_splitting << " j " << j << endl;
       ...
       {
        h2recoall->Fill(log(R/theta_reco.at(j)),log(1./z_reco.at(j) ), jetPtCorrected ,  weight);
	 ...
	 dR_max = dR;
       ...
       {
         if ( log(1./z_gen.at(l) ) < 0.693147 || log(1./z_gen.at(l)) > 0.693147+0.27726*19 ) continue;
      ...
      if (splitPass == true  ) h2smeared->Fill(log(R/theta_reco.at(index_reco)) , log(1./z_reco.at(index_reco) )  , jetPtCorrected , weight);
      if (splitDoesNotPass == true  )
      {
       h2true->Fill(log(R/theta_gen.at(index_true)) , log(1./z_gen.at(index_true)), jet_gen.pt(), weight);
       theta_response->Fill(log(R/theta_gen.at(index_true)), log(R/theta_reco.at(index_reco)), weight );
       kT_response->Fill(log(1./z_gen.at(index_true)) , log(1./z_reco.at(index_reco) ) , weight );
       kT_gen_spectrum->Fill(log(1./z_gen.at(index_true)), weight);
       theta_gen_spectrum->Fill( log(R/theta_gen.at(index_true)) , weight);
       response.Fill(log(R/theta_reco.at(index_reco)) , log(1./z_reco.at(index_reco) )  , jetPtCorrected,  log(R/theta_gen.at(index_true)) , log(1./z_gen.at(index_true) ) , jet_gen.pt(),  weight);
       if ( log(kT_gen.at(index_true)) > 3 ) residualKt->Fill( (kT_reco.at(index_reco) -kT_gen.at(index_true))/kT_gen.at(index_true)  ,weight);
       if ( log(kT_gen.at(index_true)) > log(R) && (kT_reco.at(index_reco)/theta_reco.at(index_reco)) / ( kT_gen.at(index_true)/theta_gen.at(index_true) ) < 0.3 ) zTrue->Fill(z_gen.at(index_true),weight );
      }
...
//    TFile *fout=new TFile (Form("/grid_mnt/data__data.polcms/cms/cbaldene/gen_level_angularities_debug2/C10620p1/src/LundPlane_LLR/AnalysisFW/python/UnfoldingTest/RooUnfold/ResponseHerwig7_%s_%i_%s_pT500toInf_split30.root",tag.c_str(),fileindex,date.c_str()),"RECREATE");
    TFile *fout=new TFile (Form("/grid_mnt/data__data.polcms/cms/cbaldene/gen_level_angularities_debug2/C10620p1/src/LundPlane_LLR/AnalysisFW/python/UnfoldingTest/RooUnfold/ReadResponseATLASLike%sV2%s_%i_JEC%s_JER%s_PU%s_%s_%i.root", tag.c_str(), mc_name.c_str(), year,jec_option.c_str(),jer_option.c_str(), pu_option.c_str(), algo.c_str(), fileindex ),"RECREATE");
  fout->cd();
...
#ifndef __CINT__
int main () { RooUnfoldReadResponse_3D_ATLASlike(); return 0; }  // Main program when run stand-alone
#endif
