...
void RooUnfoldReadSplitResponse_3D(std::string cFiles2="",std::string tag = "", std::string date = "", int fileindex=0)
{
...
//         xbins[10]=3.33333333;
       ...
       //  xbins_true[12]=3.;
         ...
         const int bin_det_pTjet = 2;
         const int bin_true_pTjet = 3;
...
// matching efficiency
   ...
   TH3D *h2efficiency(0);
   h2efficiency=new TH3D("efficiency","efficiency",bin_det_lntheta-1, xbins, bin_true_lnkt-1,ybins_true, bin_true_pTjet-1, pT_true);


   //true correlations with det-level cuts
   TH3D *h2true(0);

    h2true=new TH3D("true","true",bin_det_lntheta-1, xbins,bin_true_lnkt-1,ybins_true, bin_true_pTjet-1, pT_true);
    TH3D *h2fulleff(0);

     h2fulleff=new TH3D("truef","truef",bin_det_lntheta-1, xbins,bin_true_lnkt-1,ybins_true, bin_true_pTjet-1, pT_true);
   ...
   theta_gen_spectrum = new TH1D("theta_gen_spectrum", "theta_gen_spectrum",bin_det_lntheta-1, xbins);
...
//    if (jet1.looseID() == false ) cout << "fake jet" << endl;
    ...
    if (!(jet1.pt() > 550 && jet2.pt() > 550 && fabs(jet1.y()) < 2 && fabs(jet2.y()) <2 && jet1.pt() < 4000 && jet2.pt() < 4000 )) continue;
...
//     if  (!(z_gen.size()> 0 && jet.pt() > 500 && fabs(jet_gen.y()) < 2.0 && z_reco.size() > 0 && fabs(jet_gen.eta()) < 2.0  ) ) continue;
      ...
      n_jets++;
      n_splittings = 0;
     if (splitDoesNotPass) pT_response.Fill(jet_reco.pt(), jet_gen.pt(),  weight);
     if (splitPass) pTjet_det->Fill(jet_reco.pt(), weight);
     if (splitDoesNotPass) pTjet_true->Fill(jet_gen.pt(),weight);
...
//      cout << "gen-level splitting " << i << endl;
 //     cout << "=================== " << i << endl;
      ...
      if (splitDoesNotPass )  h2fulleff->Fill(log(0.4/theta_gen.at(i)) , log(kT_gen.at(i)), jet_gen.pt(), weight );
      n_splittings++;
//       histosTH2F["Lund_plane_gen_all"]->Fill(log(0.4/theta_gen.at(i)),log(kT_gen.at(i) ));
      ...
      // cout << " first splitting " << first_splitting << " j " << j << endl;
       ...
       {
        h2recoall->Fill(log(0.4/theta_reco.at(j)),log(kT_reco.at(j) ), jet_reco.pt(),  weight);
	 ...
	 dR_max = dR;
      ...
      if (splitPass == true  ) h2smeared->Fill(log(0.4/theta_reco.at(index_reco)) , log(kT_reco.at(index_reco) )  , jet_reco.pt(), weight);
      if (splitDoesNotPass == true  )
      {
       h2true->Fill(log(0.4/theta_gen.at(index_true)) , log(kT_gen.at(index_true)), jet_gen.pt(), weight);
       ...
       response.Fill(log(0.4/theta_reco.at(index_reco)) , log(kT_reco.at(index_reco) )  , jet_reco.pt(),  log(0.4/theta_gen.at(index_true)) , log(kT_gen.at(index_true) ) , jet_gen.pt(),  weight);
...
//     histosTH2F["Lund_plane_gen_matched"]->Fill(log(0.4/theta_gen.at(index_true)),log(kT_gen.at(index_true) ));
 ...
 TFile *fout = new TFile("/grid_mnt/data__data.polcms/cms/cbaldene/gen_level_angularities_debug2/C10620p1/src/LundPlane_LLR/AnalysisFW/python/UnfoldingTest/RooUnfold/ResponseIsSplit3DHerwig7.root","RECREATE");
...
#ifndef __CINT__
int main () { RooUnfoldReadSplitResponse_3D(); return 0; }  // Main program when run stand-alone
#endif
