...
void RooUnfoldReadData_3D(std::string cFiles2 = "",std::string tag = "", std::string date = "", int fileindex=0,  int year = 2018, std::string era = "B",  std::string algo = "")
{
...
//      if (events->fired(0) == 8 && year == 2016 ) HLT_450PFjet = true;      
      ...
      if (algo == "ak4" && events->fired(0) == 8 && year == 2016 ) HLT_450PFjet = true;
//    }
...
//      if (1 == 1) continue;
    ...
    if ( HLT_450PFjet == false && algo == "ak4" ) continue;
...
//    if (jet1.looseID() == false || jet2.looseID() == false ) continue;
    if (jet1_pT_corrected > 700 && jet2_pT_corrected > 700 && fabs(jet1.y()) < 1.7 && fabs(jet2.y()) < 1.7 && jet1_pT_corrected < 4000 && jet2_pT_corrected < 4000 )
    {
...
//      if  (!(   z_reco.size() > 0 && jetPtCorrected > 550 && fabs(jet_reco.y()) < 1.7 && jet_reco.looseID() == true && jetPtCorrected < 4000 ) ) continue;
      ...
      for (unsigned j = 0; j < z_reco.size(); ++j) //loop over reco-level splittings
      {
       ...
       n_splittings++;
       h2raw->Fill(log(R/theta_reco.at(j)) , log(kT_reco.at(j) ) , jetPtCorrected );
       h2rawTrueBinning->Fill(log(R/theta_reco.at(j)) , log(kT_reco.at(j) ) , jetPtCorrected );
      }
...
//    TFile *fout=new TFile ("/grid_mnt/data__data.polcms/cms/cbaldene/gen_level_angularities_debug2/C10620p1/src/LundPlane_LLR/AnalysisFW/python/UnfoldingTest/RooUnfold/rawData.root","RECREATE");
    TFile *fout=new TFile (Form("/grid_mnt/data__data.polcms/cms/cbaldene/gen_level_angularities_debug2/C10620p1/src/LundPlane_LLR/AnalysisFW/python/UnfoldingTest/RooUnfold/ReadDataControlData%i_%s_%s_%i_%s.root",year, tag.c_str(),era.c_str(), fileindex, algo.c_str() ),"RECREATE");
    fout->cd();
