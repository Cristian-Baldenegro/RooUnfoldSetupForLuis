...
void RooUnfoldSplitResponse_GJ(std::string cFiles2="",std::string tag = "", std::string date = "", int fileindex=0, double split_frac=0.5)
{
  ...
  for(Int_t m=0;m<phoEt->size();m++){
   ...
   if(phoEt->at(ing)<Emin_gamma) continue;
    ...
    if(xj_true>xjmax_true || xj_true<xjmin_true) continue;
      ...
      if(xj_det>xjmax_det || xj_det<xjmin_det) continue;
       ...
       else{response.Fill(rg_det,xj_det,rg_true,xj_true,scale);}
   }
   ...
   TFile *fout=new TFile (Form("/data_CMS/cms/cbaldene/UnfoldingTest/RooUnfold/test/ResponseIsSplit_total_%s_%i_%s_split_%f.root",tag.c_str(),fileindex,date.c_str(),split_frac),"RECREATE");
 fout->cd();
...
#ifndef __CINT__
int main () { RooUnfoldSplitResponse_GJ(); return 0; }  // Main program when run stand-alone
#endif
