...
void RooUnfoldReadResponse_LundPlaneZ(std::string cFiles2="",std::string tag = "", std::string date = "", int fileindex=0)
{
...
//      cout << "gen-level splitting " << i << endl;
 //     cout << "=================== " << i << endl;
      ...
      if (log(1./z_gen.at(i)) < 0.69 || log(1./z_gen.at(i)) > 6.0) continue; //gen-level phase-space
      if (log(0.4/theta_gen.at(i)) < 0. || log(0.4/theta_gen.at(i)) > 3.67) continue; //gen-level phase-space
      h2fulleff->Fill(log(0.4/theta_gen.at(i)) , log(1./z_gen.at(i)) );
...
//       if (mismatch == true) cout << "reco " << log(0.4/theta_reco.at(j)) << "\t" << log(kT_reco.at(j)) << "\t" << phi2_reco.at(j) << "\t" << eta2_reco.at(j) << "\t" << endl;      
       ...
       {
        h2recoall->Fill(log(0.4/theta_reco.at(j)),log(1./z_reco.at(j) ));
...
//         cout << " we have a match with dR " << dR << " and kTratio " << kT_min << endl;
       ...
       {
         if (log(1./z_gen.at(l)) < 0.69  || log(1./z_gen.at(l)) > 6.0) continue;
         if (log(0.4/theta_gen.at(l)) < 0. || log(0.4/theta_gen.at(l)) > 3.67 ) continue;
      ...
      if ( index_true != index_true_mc ) continue;
       ...
       h2smeared->Fill(log(0.4/theta_reco.at(index_reco)) , log(1./z_reco.at(index_reco) )  );
       h2true->Fill(log(0.4/theta_gen.at(index_true)) , log(1./z_gen.at(index_true)));
       response.Fill(log(0.4/theta_reco.at(index_reco)) , log(1./z_reco.at(index_reco) )  , log(0.4/theta_gen.at(index_true)) , log(1./z_gen.at(index_true) ) );
...
#ifndef __CINT__
int main () { RooUnfoldReadResponse_LundPlaneZ(); return 0; }  // Main program when run stand-alone
#endif
